...
#' 
mi_fam_d_hel <- dist.ldc(mc_ephrb, "hellinger", silent = T)
...
#' 
mi_fam_jac <- vegdist(mc_ephrb, method = 'jac', binary = T)
...
#' 
betadiver(mc_ephrb, method = 'j') %>% tidy
#' 
...
#' 
mi_fam_sor <- vegdist(mc_ephrb, method = 'bray', binary = T)
mi_fam_sor %>% tidy
