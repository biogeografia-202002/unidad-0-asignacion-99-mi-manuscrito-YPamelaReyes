...
#' 
#' MAYÚSCULAS DEL TALI (NO ES GRITAR)
#' TEXTO ORIGINAL DEL TALI: Tal como comenté arriba, variables con valores VIF por encima de 10, deben ser examinadas. En el arreglo actual, parecen estar asociadas `Ca`, y `Mg` y, por otra parte, `Zn` y `K`. Es preferible sacar una de cada par, para que la retenida explique la varianza correspondiente en cada caso. Entre `Mg` y `Ca`, no tengo preferencia por razones biogeoquímicas, así que excluyo a `Ca` por tener mayor VIF. No obstante, entre `K` y `Zn`, sin duda prefiero excluir a `K`, puesto que el zinc participa en la síntesis de proteínas de las plantas. Finalmente, nótese que `pH` y `N` lucen asociadas en este arreglo, pero esto podría deberse a la introducción de nuevas variables o a la colinealidad preexistente, así que las conservo para decidir con ellas más adelante.
#' TEXTO APLICABLE A TI: Dado que K es la que mayor VIF presenta, comenzaré sacándola a ella. El potasio suelo ser importante para las plantas, pero en este caso, dado su alto VIF, luce apropiado excluirla.
#' 
...
#' 
#' TEXTO ORIGINAL DEL TALI: Nota que las posiciones rotaron, no así la asociación entre la mayoría de las variables. `B` ahora luce asociado con `N.min.`, pero este último es muy importante para las plantas, por lo que excluiré al primero, que además presenta VIF más alto. Por otra parte, `Fe` y `Mg` interactúan fuertemente en este arreglo, por lo que excluiré `Mg` (con VIF más alto).
#' TEXTO APLICABLE A TI: Dirige tu atención a los VIF obtenidos por `vif.cca(mi_fam_hel_rda_selec2)`. Nota que la variable `UTM.EW` tiene un VIF grande. Es recomendable sacarla; nota además en el gráfico que ella forma un ángulo agudo con `N.min`, por lo que sin duda mejor sacarla a ella que a una variable de suelo. Además, `UTM.EW` forma ángulo de 180 grados con `Al`, por lo que es perfectamente inversa con éste elemento en el suelo. La recomendación es sin duda, excluir a `UTM.EW`.
#' 
...
#'
#' TEXTO ORIGINAL DEL TALI: Finalmente, la coordenada `UTM.EW` tiene un alto valor VIF, por lo que es preferible excluirla; su retiro mejorará los VIF de las demás variables, como por ejemplo `Zn` y `N.min.`.
#' TEXTO APLICABLE A TI: Ahora nota que al excluir a `UTM.EW`, ocurre que todas las variables se reacomodan, y aunque antes `riqueza_global` y `Al` no estaban plenamente superpuestas, resulta que ahora sí. Ambas variables, `riqueza_global` y `Al` tienen VIF bajos, pero están demasiado superpuestas, por lo que es aconsejable sacar una. ¿Cuál? Entre una variable de suelo y una no de suelo, mejor sacar la última: `riqueza_global` va pa'fuera.
#' 
...
#' 
#' TEXTO ORIGINAL DEL TALI: Las variables `N.min.` y `Zn` ahora presentan valores VIF aceptables. Habrás notado que es todo un arte la selección interactiva de variables; si este enfoque te parece complejo o arbitrario, hay alternativas semiautomáticas. Tal como comenté anteriormente, es posible localizar modelos de manera algorítmica usando la función `step` (y otros métodos de la misma función, aunque en `{vegan}` se usa `ordistep`), ahorrando algunos pasos, pero su desarrollo supera el alcance de esta guía. Considero válido (y oportuno) buscar variables de manera interactiva, puesto que la selección debe estar controlada por el criterio ecológico y humano, más que por el meramente numérico.
#' TEXTO APLICABLE A TI: Este modelo luce bien, y puedes conservalor de aquí en adelante. Aunque hay ángulos agudos entre `B`, `pH` y `N`, están algo separadas y tienen VIF bajos todas.
#'
